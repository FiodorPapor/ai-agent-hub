...
/**
 * Universal Agent Wallet SDK for CLI
...
export class UniversalWallet {
    ...
    try {
      ...
      if (!sufficient) {
        return {
          ...
          message: `Insufficient balance. Required: ${requiredAvax} AVAX, Available: ${availableBalance} AVAX`
        };
      }
  ...
  async pay(to: string, priceString: string): Promise<PaymentResult> {
    try {
      ...
      console.log(`[Payment] Sending ${ethers.formatEther(value)} AVAX to ${to}...`);
      ...
      if (error.code === 'INSUFFICIENT_FUNDS') {
        errorMessage = "Insufficient balance for transaction including gas fees";
